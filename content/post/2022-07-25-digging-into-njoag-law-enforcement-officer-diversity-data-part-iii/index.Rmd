...
```
There are [other definitions](https://en.wikipedia.org/wiki/North_Jersey) of the North Jersey cultural region. Our definition is on the expansive side, including three northernmost border counties, Hunterdon, Somerset and Middlesex, and comprises about 62% of the population.
...
ct_train_agcy %>%
  ...
  labs(title = "Log Linear residuals by officer_r_asian",
       subtitle = "(Training data only)")
```
Let's now see if we can find a ML algorithm that will turn up anything that we missed.

### ML agency models
We will use XGBoost again. Here our model and best tune:
```{r}
recipe_agcy_xgb <-
